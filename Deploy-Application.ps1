...
Try {
	...
	[string]$appName = 'PyCharm Edu'
	[string]$appVersion = '2022.1.3'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.1'
	[string]$appScriptDate = '08/19/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'pycharm64' -CheckDiskSpace -PersistPrompt

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		If(Test-Path -Path "$envSystemDrive\IDE\Pycharm Edu\bin\Uninstall.exe"){
			Write-Log -Message "Attempting to run uninstaller..." -Source 'Pre-installation' -LogType 'CMTrace'
			$exitCode = Execute-Process -Path "$envSystemDrive\IDE\Pycharm Edu\bin\Uninstall.exe" -Parameters "/S" -WindowStyle "Hidden" -PassThru
		...
		Else{
			Write-Log -Message "Installing Python..." -Source 'Pre-installation' -LogType 'CMTrace'
		...
		## <Perform Installation tasks here>
			...
			$exitCode = Execute-Process -Path "$dirFiles\pycharm-edu-2022.1.3.exe" -Parameters "/S /CONFIG=$dirFiles\edu_silent.config /LOG=$envSystemDrive\Windows\Logs\Software\PyCharmEdu\install.log /D=$envSystemDrive\IDE\PyCharm Edu" -WindowStyle "Hidden" -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		Write-Log -Message "Installing Packages..." -Source 'Uninstallation' -LogType 'CMTrace'
		$exitCode = Execute-Process -Path "$envSystemDrive\Program Files\Python310\Scripts\pip.exe" -Parameters "install numpy matplotlib scipy sympy pandas mandelbrot jupyter pillow plotly seaborn scikit-learn" -WindowStyle "Hidden" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		Write-Log -Message "Packages installed!" -Source 'Uninstallation' -LogType 'CMTrace'
		## Start-Process "C:\Program Files\Python37\Scripts\pip.exe" -ArgumentList "install numpy matplotlib scipy sympy pandas mandelbrot jupyter pillow plotly seaborn scikit-learn" -NoNewWindow
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'pycharm64' -CloseAppsCountdown 60
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5SkfLmR6HAPcDx/fQFBDSRD8
# rDSgghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFIgsgZXOOpkMAy0nKHzcV3BFqjb7MA0GCSqGSIb3DQEBAQUABIIBgFsblCgK
# jKAXKwHDR2zMl1lHv9ow3Nk8IxlATfSVuwrWXrJw3Mx9H8kZnm1GgIpqrm5Z7o0W
# WM7pWcFqR1gudRHerV3bAuEhzZVZDFjtEr85ohssMUGx7aCwFQLV+GM8qxsxA+1l
# Z3bDSJqxAcLGZd3sdm8F4fKvgbM+2MU7aRofB+gjAFgcC/sqa3oGU58NF1xdZBgC
# 2bJ9YoQOkAACmrz6BjwyAgAi7VEx1LEndHxf/D5jpP6LoIK+5qdXs9UbPo4Gm3cx
# LtayWA1scbQQY57jmTKjFxvNUj+PY93wC7PE0hoQGvCXfZ8qnaE6UPJpkSk6G6zy
# KdiJaI65rG7obMLRCG4EJbV3At9rk5b8ScgtnHHdGl1J7LGqJyFyLDuXkpX69FWM
# qtHwK3QSdy1Hf3S7qFSKThIYGzaQ8HSFyt5VGS7jSbMJNoSIIoGCblLGX9WKvvb8
# 7TZv8cCSVPuWpWMLDfvYIXJZnkaA0nhzuaH6Gzl5X1qPiaPubRKuyKpc2w==
